...
def open_by_reconstruction(src, iterations = 1, ksize = 3, debug = False):
        ...
        if debug:
            cv2.imshow('reconstruction', src)
            cv2.waitKey(0)
        eroded = cv2.erode(src, np.ones((5,5), np.uint8), iterations=4)
        if debug:
            cv2.imshow('reconstruction - eroded', eroded)
            cv2.waitKey(0)
...
def reconstruct_skeleton(src, orig,  ksize = 3, debug = False):
    this_iteration = src
    last_iteration = src
    for i in range(15):
    ...
    if debug:
        cv2.imshow('reconstruction - pruned', skeleton)
        cv2.waitKey(0)
    
    return skeleton

def cluster_skeleton(skeleton):
...
def remove_small_blobs(img, min_blob_size, debug=False):
    if debug:
        cv2.imshow('Small blobs removed input', img)
        cv2.waitKey(0)
    ...
    if debug:
        cv2.imshow('Small blobs removed output', output)
        cv2.waitKey(0)
